...
def sync_detailed(
    *,
    ...
    Example:
    ```bash
    curl -X GET \"http://localhost:4000/v1/containers?limit=20&order=desc\"         -H \"Authorization:
    Bearer sk-1234\"
    ```

    Or specify provider via header or query param:
    ```bash
    curl -X GET \"http://localhost:4000/v1/containers?custom_llm_provider=azure\"         -H
