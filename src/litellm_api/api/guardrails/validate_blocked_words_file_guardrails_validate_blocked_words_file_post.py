from http import HTTPStatus
from typing import Any, cast
from urllib.parse import quote

import httpx

from ...client import AuthenticatedClient, Client
from ...types import Response, UNSET
from ... import errors

from ...models.http_validation_error import HTTPValidationError
from ...models.validate_blocked_words_file_guardrails_validate_blocked_words_file_post_request import ValidateBlockedWordsFileGuardrailsValidateBlockedWordsFilePostRequest
from typing import cast



def _get_kwargs(
    *,
    body: ValidateBlockedWordsFileGuardrailsValidateBlockedWordsFilePostRequest,

) -> dict[str, Any]:
    headers: dict[str, Any] = {}


    

    

    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/guardrails/validate_blocked_words_file",
    }

    _kwargs["json"] = body.to_dict()


    headers["Content-Type"] = "application/json"

    _kwargs["headers"] = headers
    return _kwargs



def _parse_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Any | HTTPValidationError | None:
    if response.status_code == 200:
        response_200 = response.json()
        return response_200

    if response.status_code == 422:
        response_422 = HTTPValidationError.from_dict(response.json())



        return response_422

    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Response[Any | HTTPValidationError]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    *,
    client: AuthenticatedClient,
    body: ValidateBlockedWordsFileGuardrailsValidateBlockedWordsFilePostRequest,

) -> Response[Any | HTTPValidationError]:
    r""" Validate Blocked Words File

     Validate a blocked_words YAML file content.

    Args:
        request: Dictionary with 'file_content' key containing the YAML string

    Returns:
        Dictionary with 'valid' boolean and either 'message'/'errors' depending on result

    Example Request:
    ```json
    {
        \"file_content\": \"blocked_words:\n  - keyword: \\"test\\"\n    action: \\"BLOCK\\"\"
    }
    ```

    Example Success Response:
    ```json
    {
        \"valid\": true,
        \"message\": \"Valid YAML file with 2 blocked words\"
    }
    ```

    Example Error Response:
    ```json
    {
        \"valid\": false,
        \"errors\": [\"Entry 0: missing 'action' field\"]
    }
    ```

    Args:
        body (ValidateBlockedWordsFileGuardrailsValidateBlockedWordsFilePostRequest):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Any | HTTPValidationError]
     """


    kwargs = _get_kwargs(
        body=body,

    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)

def sync(
    *,
    client: AuthenticatedClient,
    body: ValidateBlockedWordsFileGuardrailsValidateBlockedWordsFilePostRequest,

) -> Any | HTTPValidationError | None:
    r""" Validate Blocked Words File

     Validate a blocked_words YAML file content.

    Args:
        request: Dictionary with 'file_content' key containing the YAML string

    Returns:
        Dictionary with 'valid' boolean and either 'message'/'errors' depending on result

    Example Request:
    ```json
    {
        \"file_content\": \"blocked_words:\n  - keyword: \\"test\\"\n    action: \\"BLOCK\\"\"
    }
    ```

    Example Success Response:
    ```json
    {
        \"valid\": true,
        \"message\": \"Valid YAML file with 2 blocked words\"
    }
    ```

    Example Error Response:
    ```json
    {
        \"valid\": false,
        \"errors\": [\"Entry 0: missing 'action' field\"]
    }
    ```

    Args:
        body (ValidateBlockedWordsFileGuardrailsValidateBlockedWordsFilePostRequest):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Any | HTTPValidationError
     """


    return sync_detailed(
        client=client,
body=body,

    ).parsed

async def asyncio_detailed(
    *,
    client: AuthenticatedClient,
    body: ValidateBlockedWordsFileGuardrailsValidateBlockedWordsFilePostRequest,

) -> Response[Any | HTTPValidationError]:
    r""" Validate Blocked Words File

     Validate a blocked_words YAML file content.

    Args:
        request: Dictionary with 'file_content' key containing the YAML string

    Returns:
        Dictionary with 'valid' boolean and either 'message'/'errors' depending on result

    Example Request:
    ```json
    {
        \"file_content\": \"blocked_words:\n  - keyword: \\"test\\"\n    action: \\"BLOCK\\"\"
    }
    ```

    Example Success Response:
    ```json
    {
        \"valid\": true,
        \"message\": \"Valid YAML file with 2 blocked words\"
    }
    ```

    Example Error Response:
    ```json
    {
        \"valid\": false,
        \"errors\": [\"Entry 0: missing 'action' field\"]
    }
    ```

    Args:
        body (ValidateBlockedWordsFileGuardrailsValidateBlockedWordsFilePostRequest):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Any | HTTPValidationError]
     """


    kwargs = _get_kwargs(
        body=body,

    )

    response = await client.get_async_httpx_client().request(
        **kwargs
    )

    return _build_response(client=client, response=response)

async def asyncio(
    *,
    client: AuthenticatedClient,
    body: ValidateBlockedWordsFileGuardrailsValidateBlockedWordsFilePostRequest,

) -> Any | HTTPValidationError | None:
    r""" Validate Blocked Words File

     Validate a blocked_words YAML file content.

    Args:
        request: Dictionary with 'file_content' key containing the YAML string

    Returns:
        Dictionary with 'valid' boolean and either 'message'/'errors' depending on result

    Example Request:
    ```json
    {
        \"file_content\": \"blocked_words:\n  - keyword: \\"test\\"\n    action: \\"BLOCK\\"\"
    }
    ```

    Example Success Response:
    ```json
    {
        \"valid\": true,
        \"message\": \"Valid YAML file with 2 blocked words\"
    }
    ```

    Example Error Response:
    ```json
    {
        \"valid\": false,
        \"errors\": [\"Entry 0: missing 'action' field\"]
    }
    ```

    Args:
        body (ValidateBlockedWordsFileGuardrailsValidateBlockedWordsFilePostRequest):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Any | HTTPValidationError
     """


    return (await asyncio_detailed(
        client=client,
body=body,

    )).parsed
